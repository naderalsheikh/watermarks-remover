...
### PR 46 -- Bugfix: `docProps/custom.xml` drop wrongly failed `part_inventory` under `external_sharing` -- implemented (2026-08-28)

A real user-reported release failure, investigated as a correctness bug rather than user error, per the report's own framing. Reported symptom: releasing `Joint Revocable Living Trust Agreement Consolidated Master Draft (2).docx` under `counterparty_deal_room` completed `failed`, both the job's error and `release_result.reason` reading `verification failed: part_inventory`; direct engine reproduction isolated it to `apply_actions` dropping `word/comments.xml` and `docProps/custom.xml`, with `verify_derivative`'s `part_inventory` check reporting `added=[] dropped=['docProps/custom.xml', 'word/comments.xml']` as a failure.

- **Root cause, confirmed not assumed**: `container_meta.py`'s `drop_custom_xml` flag (set from `policies.py`'s `"drop_custom_xml": a["custom_xml"] == "strip"`, true under `external_sharing`/`production`) deliberately drops `docProps/custom.xml` (custom *document properties* -- e.g. a firm's "Matter Number" field) the same way, and for the same reason, it drops `customXml/*.xml` tree parts (custom XML *data storage*) -- both are arbitrary, firm-defined content this policy strips on sharing. The drop was intended; the failure was not. `verify.py`'s `_ALLOWED_DROP_TAGS` allowlists dropped parts by case-insensitive substring match against the full part path, and its `"customxml"` fragment matches `customxml/item1.xml` (no separator) but not `docprops/custom.xml` -- the literal `.` between "custom" and "xml" breaks the substring match. A correct, policy-intended drop was flagged as unexplained part loss.
- **Fix, narrow as requested**: `_ALLOWED_DROP_TAGS` (`verify.py`) gains one new entry -- `"docprops/custom.xml"`, the full expected path, not a broadened `"customxml"` fragment or a bare `"custom.xml"` fragment that could match an unrelated part. Confirmed directly (not just by the passing test) that `docProps/core.xml`, `docProps/app.xml`, `word/settings.xml`, and `[Content_Types].xml` still fail to match -- a drop of any of those still fails verification, as it should. `strict` policies (`privacy_only`/`evidence_preservation`) are untouched: they reject *any* dropped part, allowlisted or not, so this fix has no effect there by design.
- **Regression test**: `tests/test_verify.py::test_docprops_custom_xml_drop_passes_inventory_under_external_sharing` -- a real DOCX carrying `docProps/custom.xml` (a "Matter Number" custom property, not a synthetic placeholder), run through the actual `inspect_bytes` -> `plan_actions` -> `apply_actions` -> `verify_derivative` pipeline under `external_sharing`. Confirmed directly, not assumed, that this exact fixture fails `part_inventory` against the pre-fix allowlist and passes against the fixed one.

Full backend suite green (1209 collected, up from 1208, zero regressions). `ruff check service/scripts/verify.py tests/test_verify.py` clean. End-to-end verified against a real, isolated scratch backend (never the operator's own): uploaded a DOCX named with the reporter's own filename, carrying `docProps/custom.xml` and a comment, and prepared a real release through `POST .../documents/{id}/releases` under `counterparty_deal_room` -- release and job status both went from the reported `failed` to `done`, `release_result.reason` empty.

### PR 47 -- Evaluation coherence pass: refusal copy, Release-first links, packet filenames, ReleasePanel legibility, demo-matter note -- implemented (2026-08-28)

Six small, targeted fixes from a live-demo-experience friction audit; no new architecture. (Backfilled into the changelog on 2026-08-30 -- the commit, `9841e47`, landed between PR 46 and PR 49 but its entry was never written at the time.)

- **Refusal copy stopped overclaiming**: PR 45's "Refused by policy -- this is expected, not an error" heading was accurate for a deliberate policy refusal (macros, an unattested signature) but not for a PDF hitting the engine's not-yet-implemented content-strip paths (`pdf_annots`/`pdf_attachments`/`pdf_js_actions` under `policies.py`'s `_apply_pdf`) -- which also lands a job in `refused`. The heading now reads "Refused by policy -- no derivative was produced" and asserts intent only where the policy genuinely made a decision. (PR 48, below, gives this second refusal class its own dedicated framing.)
- **Job-history links say "Release" when the job was one**: the matter view's job-history disclosure labeled every job "Sanitize" regardless of outcome because `j.kind` is always `"sanitize"` for a release-wrapped job; it now renders "Release" when `release_id` is present, mirroring the job page's own H1 logic.
- **Release-first vocabulary sweep**: remaining "sanitized"/"sanitize" copy replaced across the job page (packet heading, bundle-contents list), the matter view page (help text, empty state), and the dashboard (Recent Activity label, attention-item copy).
- **Packet download filenames name the document**: `job_bundle`'s `Content-Disposition` used to be a raw `{job.id}-release-packet.zip` hex string, indistinguishable from any other job's download once saved. New `_safe_download_stem` (`service/app/main.py`) builds the filename from the document's own upload name (stem, control characters and quote/backslash replaced, capped at 80 chars) plus the job id. `tests/test_app.py::test_bundle_filename_sanitizes_a_hostile_document_name` confirms a crafted upload filename can't inject a second Content-Disposition field; the existing bundle filename assertion was updated for the new scheme.
- **ReleasePanel legibility**: the signature-attestation checkbox -- irrelevant for the large majority of documents, which carry no signature, and unknowable in advance without an inspect job -- moved into the existing "Technical details" disclosure instead of competing with the primary choices; Recipient is now marked "(required)" for symmetry with its "(optional)" siblings.
- **Demo-matter persistence note**: a demo matter now shows a note above Upload warning that uploads there persist for every future walkthrough, nudging toward a new matter instead; the eval runbook's "try your own document" step (step 8) was updated to match.

Full backend suite: 1209 collected, one failure encountered under sustained heavy system load (`test_release_model.py::test_batch_release_mixed_outcome`, a TimeoutError) -- the pre-existing, documented BatchDispatcher polling flake (PR 44's own changelog names this class), reproduced clean in isolation immediately after, and unrelated to this pass (no dispatcher/batch code touched). `tests/test_app.py` ran clean in full separately. Frontend gates green (`tsc`, `eslint`, `vitest` 46/46, `next build`). Live-verified end to end against an isolated scratch backend + frontend: sample-matter seed, one done release, one refusal, and one fresh user-uploaded DOCX released through a newly-created (non-demo) matter -- all six fixes confirmed rendering.

### PR 48 -- Policy honesty fix: PDF annotations/attachments/active content refuse instead of claiming a strip -- implemented (2026-08-28)

The policy table said `"strip"` for `pdf_annots`/`pdf_attachments`/`pdf_js_actions` under `external_sharing`/`production`, but the engine has no PDF object-graph editor at all -- no code anywhere touches `/Annots`, `/EmbeddedFiles`, `/OpenAction`, `/JS`, `/AA`, or `/AcroForm` -- so every real strip request for this content already ended in `_apply_pdf`'s own refusal. The policy row claimed an action the engine never performed. (This is the fix the first custody review later leaned on when auditing refusal honesty. Backfilled into the changelog on 2026-08-30 -- commit `9fe058b` landed between PR 46 and PR 49 but its entry was never written at the time.)

- **Honest labels, zero behavior change**: the three rows now say `"refuse"` instead of `"strip"` -- the refusal already happened, only the label lied. `_apply_pdf`'s check now matches `"strip"` **or** `"refuse"` so a custom policy overlay that still sets one of these to `"strip"` keeps refusing rather than silently succeeding. `production`'s approve-path for `pdf_annots`/`pdf_attachments` (`_APPROVE_RESOLVES_TO`, derived from `external_sharing`'s own row) picks up the fix automatically -- an operator who clicks "Approve (strip)" gets an honestly-labeled refusal, not a doomed silent strip. `pdf_acroform` is kept in the refusal check defensively for custom overlays even though no default policy row reaches it.
- **A refusal class of its own**: new `PDF_CONTENT_REFUSAL_MARKER = "pdf content removal not implemented"` constant in `service/scripts/policies.py`, with a clearer message using `report_html.SUBTYPE_LABELS` for human-readable content names, distinguishes "the engine can't do what this policy demands" from a deliberate policy decision (macros, an unattested signature). The job page renders two different framings for `refused` matched via this marker: a capability gap ("this policy would require removing [content], but that removal isn't implemented yet") vs. policy-by-design ("declined to produce a derivative ... rather than ship an incomplete or unsafe result"). The marker is a narrow string kept in sync between `policies.py` and the frontend's copy -- deliberately not a structured reason code (`job.error` is a plain string end-to-end today; a real reason-code field would need a Job column/migration), and flagged as technical debt in both places' comments.
- **The previously untested path got tests**: new `tests/test_pdf_content_refusal.py` (10 tests) -- the five requested fixtures (annotated, embedded-file, JavaScript, OpenAction-only, AA-only PDFs) under `external_sharing`, plus confirmation that `production`'s "Approve (strip)" route doesn't silently succeed for `pdf_annots`/`pdf_attachments`, a clean-PDF baseline, and a check that the new marker doesn't leak into unrelated refusal messages. Before this, the refusal path had zero regression coverage -- only the Finding projection was tested.

Full backend suite: 1219 collected, one failure under sustained extreme system load (load average peaked ~170 on an 8-core machine) -- `test_batches.py::test_batch_sanitize_mixed_outcomes_are_per_document`, the same `_wait_batch_done` BatchDispatcher-polling mechanism PR 47 hit in a sibling test; confirmed via isolated rerun once load cleared, unrelated to this pass. This was the second occurrence of this flake class across two different tests -- tracked as real, recurring test debt (PR 49's dispatcher hardening is the response). Every directly-relevant test file (`test_pdf_legal.py`, `test_verify.py`, `test_pdf_structural_rewrite.py`, `test_pdf_deep_images.py`, `test_refuse_list.py`, `test_legal_corpus.py`, `test_policies.py`, `test_report_html.py`) ran clean in full separately before the full-suite run. Frontend gates green (`tsc`, `eslint`, `vitest` 46/46, `next build`). `ruff` clean.

### PR 49 -- BatchDispatcher test flake hardening: lazy startup, deterministic cleanup, completion reconciliation -- implemented (2026-08-28)

Follows the recurring full-suite failures in `tests/test_batches.py` / `tests/test_release_model.py` where real worker subprocesses could remain `running` past a fixed 10-second polling window under heavy system load. The issue was release-gate confidence, not product semantics: the batch endpoint itself eventually completed, but the test harness and dispatcher lifecycle amplified load and produced noisy full-suite failures.

- **Dispatcher startup is lazy/idempotent** (`service/app/dispatcher.py`): `start()` is safe to call more than once; `wake()` starts the dispatcher as a fallback for tests that construct `TestClient(create_app(...))` without ASGI lifespan startup. `create_app()` no longer starts the dispatcher at app construction time; production ASGI startup starts it, and non-batch tests no longer create idle dispatcher threads simply by constructing an app.
- **Deterministic test cleanup**: batch/release fixtures now explicitly stop `app.state.batch_dispatcher`, and the handful of direct `_build_env()` tests that bypass the fixture cleanup use `try/finally` to stop their dispatcher even if an assertion fails. This closes the thread-leak/load-amplification path that made unrelated full-suite runs progressively less reliable.
- **Completion reconciliation**: each dispatcher poll now checks unfinished batches whose children are already terminal and marks them complete if appropriate. `_run_one_inner()` also runs `check_batch_completion()` in a `finally` around audit/release finalization. This prevents an exception in `_append_child_audit()` or `sync_release()` from leaving a batch polling forever after its child job has already reached a terminal state.
- **Polling helpers hardened**: batch wait helpers now use a 60-second budget for real worker subprocesses, exponential backoff capped at 0.5s to reduce SQLite write contention, and timeout messages include the last observed batch payload instead of just "did not finish."

New test: `tests/test_batches.py::test_dispatcher_finalization_exception_does_not_wedge_batch` proves a finalization exception does not prevent batch completion. Focused `tests/test_batches.py tests/test_release_model.py` passed (40/40). Full backend suite passed from the current tree (1221 collected: 1220 passed, 1 pre-existing scipy skip, 0 failures). `ruff check service/app/dispatcher.py service/app/main.py tests/test_batches.py tests/test_release_model.py` clean. No frontend files touched.

### PR 50 -- Commercial surface manifest: CounselClear product vs. research harness boundary -- documented (2026-08-28)

Adds `docs/COUNSELCLEAR_COMMERCIAL_SURFACE.md`, a repo-grounded classification document derived from actual current paths rather than strategy-only language. It does **not** move or delete code, alter workflows, or change what ships; it records the current boundary and the current mismatches so a later enforcement pass has a stable baseline.

- **What is explicitly in the CounselClear commercial surface**: `service/app/`, `service/Dockerfile.counselclear`, `web/`, `tools/counselclear_airlock.py`, `tools/counselclear_verify_release_packet.py`, and the `compose.yaml` `legal`/`pg` profile services (`cc-api`, `cc-worker`, `cc-postgres`, `cc-freshclam`). This is the Release Gate product as it actually exists today.
- **What is explicitly out**: `clean_ctrlregen.py` / `Dockerfile.ctrlregen` / `setup_ctrlregen.*` / `requirements-ctrlregen.txt` and `score_synthid.py` / `synthid_score_server.py` / `Dockerfile.synthid` / `setup_synthid.*` / `requirements-synthid-scorer.txt` are classified as hard-block or build-exclude based on the repo's own existing diligence position (no upstream LICENSE / non-commercial research license). `detect_text_watermark.py`, `text_detectors.py`, `rewrite_text.py`, `markdiffusion_harness.py`, their bootstrap files, and the `wr-markllm` / `wr-markdiffusion` compose services are classified as quarantine + legal-review-needed, not product-core.
- **Most important repo-level mismatch captured, not hand-waved**: the repo still has a live release automation path publishing `markllm` and `markdiffusion` images (`.github/workflows/release-images.yml`) while the product strategy has already moved away from watermark-removal as the commercial spine. The manifest names this as the clearest current build-surface inconsistency.
- **Most important docs/config mismatch captured**: `README.md`, `.env.example`, `compose.yaml`'s default `wr-core` path, and `compose-check.sh` still present the upstream watermark-removal utility and harness ecosystem as the default operator surface. The manifest classifies those as `docs-remove` / `build-exclude` for a future cleanup pass, instead of pretending the repo is already CounselClear-only.

No tests or runtime behavior changed; this is a classification baseline for a later enforcement pass.

### PR 51 -- CounselClear-first packaging surface: default compose path, release-image publishing, and operator docs -- implemented (2026-08-29)

Implements the minimum enforcement pass implied by PR 50, without deleting research code or changing engine/runtime semantics. The goal of this pass is alignment: make the default deploy/operator path CounselClear-first, and stop presenting the research harnesses as part of the default commercial release surface.

- **Release-image workflow narrowed to the product image**: `.github/workflows/release-images.yml` now publishes only `service/Dockerfile.counselclear`, tagged as `ghcr.io/guillaumemeyer/watermarks-remover-counselclear:<tag>` and `:latest`. The old harness publish matrix (`markllm`, `markdiffusion`) is removed entirely, so GHCR publishing no longer treats those research images as release artifacts.
- **Compose default path flipped to CounselClear**: `compose.yaml`'s default `docker compose up --build -d` path now brings up the CounselClear stack (`cc-api`, `cc-worker`, `cc-freshclam`) instead of the upstream `wr-core` utility. `wr-core` is still present, but moved behind an explicit `profiles: [upstream]` opt-in. `cc-postgres` remains behind `pg`. Compose naming is updated from `watermarks-remover` to `counselclear` to match the default surface.
- **Default validation path narrowed**: `compose-check.sh` now validates only the default CounselClear stack via `GET /health/ready` on `http://127.0.0.1:8443` (override `COUNSELCLEAR_BASE_URL`). It intentionally stops treating `wr-markllm`, `wr-markdiffusion`, `wr-ctrlregen`, and `wr-synthid` as part of the default validation surface. `Makefile` comments and `compose-up-heavy` are updated to reflect the explicit `upstream` profile.
...
### PR 55 -- Structured legal justification for surviving findings -- implemented (2026-08-29)

Closes the evidentiary gap between "the manifest proves what happened" and "the release record states the legal basis for what survived." The existing `reason` codes in `policies.py` remain internal execution explanations (`operator_kept`, `no_decision`, `attested_signature_break`, etc.); they are deliberately not repurposed as legal conclusions.

- **Operator-supplied, not policy-derived**: callers may provide `legal_justifications: {subtype: {basis, note}}` on raw sanitize jobs and single-document Release jobs. The policy layer validates subtype names and controlled-vocabulary bases (`unspecified`, `privilege`, `work_product`, `pii_confidentiality`, `relevance`, `court_order`, `client_instruction`, `litigation_hold`, `gdpr_access`, `other`). If a surviving finding has no supplied basis, the manifest records `basis: "unspecified"` rather than inventing privilege/work-product/etc. from a policy default.
- **Job-row handoff across the process boundary**: `jobs.legal_justifications` (migration `0009`) stores the operator payload next to `finding_decisions`, then `runner.py`/`worker.py` pass it into the byte-pure engine call. The API does not import policy code to validate it; the engine/policy layer remains authoritative, preserving the control-plane vs. engine boundary.
- **Manifest/report/release artifacts now carry it**: `emit_manifest()` emits `action_records[]` alongside the legacy string `actions[]`; surviving findings (`keep`, `flag`, `inspect_only`) carry `legal_justification` on their action record. `report.json` repeats the action records, while `release_result.json` and `release_packet.json` expose a compact `legal_justifications[]` projection for reviewer/verifier consumption.
- **Schemas updated**: `manifest.schema.json`, `report.schema.json`, `release_packet.schema.json`, and `release_result.schema.json` now require the new legal-basis-bearing fields. `finding.schema.json` in both existing schema roots (`engine/schemas/` and `service/scripts/schemas/`) accepts an optional `legal_justification` field, but raw detection findings still do not require it -- the legal basis normally attaches later, when a finding survives a specific release decision. The pre-existing schema duplication is intentionally left in place for this scoped PR; consolidation is separate cleanup.

### PR 56 -- Chain-committed artifact hashes: the job.sanitize event commits what the packet later declares -- implemented (2026-08-29)

Closes custody-review finding MUST-1: every hash inside `release_packet.json` was declared by the packet itself and recomputed by the offline verifier against whatever bytes sat on disk at download time -- so a manifest altered *between job completion and packet assembly* re-hashed clean, because the packet faithfully declared the tampered file's hash. Nothing bound the packet's claimed hashes to anything outside the packet.

- **Hashes committed at the moment the manifest is first written**: `service/app/audit.py::_terminal_hash_facts` reads the manifest bytes from disk at job-terminal time (never from `result_json`'s dict -- a dict is semantically equal to the written file but not byte-equal, and the chain commits bytes, not meaning) and the `job.sanitize` audit event now carries `manifest_sha256` and `derivative_sha256`. Refused/failed jobs omit the keys entirely -- absent, not nulled, so "no bundle produced" stays distinguishable from "hash unknown". Historical chains are unaffected: `event_hash` only covers new rows, and live `verify_chain` recomputes from each stored row's own payload.
- **All three append sites, one helper**: the raw `/sanitize-jobs` route, the Release route, and the batch dispatcher's child-event path all thread `**_terminal_hash_facts(job)` into the same payload construction (a helper in `audit.py` because both `main.py` and `dispatcher.py` need it and `dispatcher.py` cannot import from `main.py`).
- **Offline verifier cross-checks the packet against the live chain**: `tools/counselclear_verify_release_packet.py --audit-csv <export.csv>` independently recomputes every exported chain row's hash (`sha256(prev_hash|seq|actor_id|action|canonical_payload)`, the same construction as `app/audit.py::event_hash`, reimplemented so the verifier stays stdlib-only), walks the prev-hash linkage and seq contiguity, finds this job's `job.sanitize` event, and compares the packet's declared manifest/derivative hashes against the chain-committed ones. A tampered chain is not a better witness than no chain: a chain that fails its own row-hash recheck fails the packet too.

Tamper simulation, both synthetic (verifier unit tests) and through the real stack: alter the on-disk manifest after job completion, pull a fresh packet (which honestly re-hashes the tampered bytes -- internally consistent), and the `--audit-csv` check fails with a per-hash MISMATCH. The internal checks alone still pass -- that gap is exactly what MUST-1 existed to close, and the test asserts both halves. Full backend suite green, `ruff` clean across service/tests/tools. Committed as `c33adb3`.

### PR 57 -- Signed release packets: Ed25519 operator signatures with offline, stdlib-only verification -- implemented (2026-08-29)

Closes custody-review finding MUST-2: nothing about the packet was signed. An attacker with filesystem access could rewrite `release_packet.json` -- swap the `matter_id`, claim a different policy, cite different audit refs -- and recompute every hash honestly over the swapped content; the offline verifier would pass it, because every check was the packet agreeing with itself. The full decision doc is `docs/counselclear-release-packet-signing.md`; the short version of each reviewed question:

- **Asymmetric Ed25519, not the existing HMAC**: the HMAC secrets are fine for server-side tokens (only the server ever verifies them), but a packet's signature is verified *offline by the recipient* -- opposing counsel's expert. HMAC hands every verifier the signing key; Ed25519 ships only the public half. The choice is disqualifying, not a tradeoff.
- **Key lifecycle**: file-based `data/auth/custody_signing_key.pem`, PKCS#8, auto-generated 0600 on first packet issuance (lazily -- a deployment that never issues a packet never writes a key), atomic write, existing-file-wins so rotation is a deliberate operator act. `key_id` = sha256(public raw bytes)[:16] travels in every signature; `--public-key` is repeatable so rotated key bundles verify old packets too. Deliberately not DB-stored: a key readable by the same DB owner/backup path as the chain it witnesses is no separation at all.
- **What gets signed**: the whole packet minus the signature block, canonicalized (`sort_keys`, compact separators, `ensure_ascii`) -- binding metadata (`release_id`/`matter_id`/`job_id`/`policy`/`audit_refs`/`legal_justifications`) to the file hashes, not just the hashes to themselves. The signature deliberately does not cover the file's pretty-print formatting, only the parsed content, so re-serialization doesn't break verification while any content change does. Byte-stability holds because the schema allows no floats. The signed `digest` field doubles as a canonicalization-drift detector distinct from an opaque Ed25519 failure.
- **`anchor` upgraded honestly**: `type: "ed25519-operator"`, `reference: key_id`, `digest: None` -- a digest of the signed bytes cannot appear inside those bytes, so the binding digest lives in the signature block one key over. `release_result.json` carries `signature_ref` (algorithm/key_id/signed_fields), not its own signature: no derivative to protect, and a second signature over overlapping facts would add a tampering surface, not a checkable claim. Combined mode cross-checks the ref's `key_id` against the packet signature's.
- **Verifier stays stdlib-only**: RFC 8032 §5.1 reference verification implemented directly in the verifier (cross-pinned against the `cryptography` library by test on every run), `--public-key <pem|hex>` (repeatable), and `--verify-signature` as the strict opt-in that escalates downgrades to failures for operators checking their own output. Status vocabulary is its own `Signature:` line -- `VERIFIED` / `UNSIGNED (packet predates signatures)` / `NOT VERIFIED (no --public-key given)` / `NOT VERIFIED (key not provided)` / `MISMATCH` -- never "VALID". Only `mismatch` fails; unsigned/no-key/unknown-key stay usable with explicit downgrade lines so legacy pre-PR-57 packets verify exactly as well as the day they were issued (the schema marks `signature` optional for that reason). `Externally anchored: no` still prints for `ed25519-operator` -- an operator signature is the producing system vouching for its own output, authenticating origin, not time.
- **`GET /v1/custody-public-key`** (unauthenticated -- public by definition): serves the PEM + key_id for the Airlock CLI/UI's convenience, with a docstring explicit that the trust root is the operator's out-of-band transmittal, not the HTTP response. The packet's own README now tells the recipient to keep the public key alongside the packet and how to verify offline.

Tamper simulations, through the real stack: a real signed packet with one flipped metadata byte (`matter_id`) -- every file hash still clean, the only net that catches it is the signature, and it fails with `MISMATCH` + "treat as tampered"; a tampered sibling `manifest.json` fails the hash check while the signature stays verified, pinning that the two nets are independent and stay that way; wrong key / no key / unsigned legacy each report their precise downgrade instead of a vague failure. Key file asserted 0600 and byte-stable across issuances. Full backend suite green (1258 passed, 1 skipped, 0 failures), `ruff` clean across service/tests/tools.

### PR 58 -- CI fix folded in alongside the custody review: release-image namespace derives from github.repository -- implemented (2026-08-29)

A two-line fix that landed between PR 55 and PR 56 (`393544c`), not part of the custody-review numbering but worth its own record. `.github/workflows/release-images.yml` hardcoded the GHCR image tag to `ghcr.io/guillaumemeyer/watermarks-remover-counselclear` -- the upstream owner's namespace. Login authenticates with `GITHUB_TOKEN` scoped to whichever repo runs the workflow, so a `v*` tag push on the fork would authenticate as one owner but try to push a package under the other's namespace, which GHCR denies. Tags now derive from `${{ github.repository }}` (lowercased), so the workflow publishes under whichever repo actually runs it. No product code touched; not separately re-verified beyond the workflow-file diff itself.

### PR 59 -- Custody-review SHOULD fixes: cancel-race guard, never-dropped audit events, certificate legal bases, dead-code deletion -- implemented (2026-08-29)

The four SHOULD-severity findings from the custody review (MUST-1/MUST-2 landed as PR 56/57), dispatched as four parallel specialists with a fail-without/pass-with proof mandate per finding, then lead-reviewed and landed as four separate commits -- kept as one changelog entry because the review produced them as one coherent list and each fix is only legible against the finding it closes.

- **SHOULD-1, cancel-race guard (`a2c8dc6`, `service/app/main.py` + `tests/test_batches.py`)**: `cancel_batch` collected still-queued child ids with a SELECT, then bulk-UPDATEd by id alone -- no status guard. If the dispatcher claimed a child (`queued` -> `running`) between the two, the UPDATE force-flipped the running job to failed anyway: a job that hadn't failed was recorded as failed, the `batch.cancelled` event's count lied, and `sync_release` then wrote the stale "failed" into the claimed job's sibling Release. The bulk UPDATE now carries `Job.status == "queued"` alongside the id filter (a row claimed mid-race is the dispatcher's to finish); `rowcount`, not the pre-SELECT list's length, is the truthful cancelled count feeding both the audit payload and the response; a post-UPDATE re-SELECT (`actually_cancelled`) gates the `sync_release` loop to only the rows this call actually flipped. The regression test (`test_cancel_batch_concurrent_claim_does_not_flip_running_job`) is deterministic: the poll loop is disabled and a SQLAlchemy `before_execute` listener flips one child to running on the app's own engine in the exact SELECT-to-UPDATE window -- fails without the guard (`'failed' == 'running'`) and passes with it: claimed child untouched, truthful counts (1 cancelled + 1 running), no premature `batch.completed`, then one truthful `batch.completed` after the claimed child finishes normally.
- **SHOULD-2, never-dropped terminal audit event (`62fb345`, `service/app/dispatcher.py` + `tests/test_batches.py`)**: `_run_one_inner`'s finalization was a bare try/except around `_append_child_audit`/`sync_release` -- an exception there was logged and swallowed permanently, so a batch child that reached a terminal status could end up with no `job.sanitize`/`job.inspect` audit event at all: `result_json`/manifest committed but the custody record of the act gone. That is evidence integrity, not a cosmetic gap; the existing PR 49 test only asserted the batch didn't wedge. `_finalize_child_audit` now retries bounded (3 attempts, exponential backoff, each attempt rolling back and re-getting Job/Batch off committed rows so a failed attempt's session state can't poison the next); on exhaustion the event is still written, degraded -- same action name, the facts the dispatcher still knows, an explicit `finalization_degraded` marker and the reason, through the same `append_event` hash-chain path as every other event, never a raw INSERT (a custody record outside the chain verifies as nothing). The degraded event's actor is `batch.requested_by` (the same principal the normal event would have -- never the matter id, which is not an actor; "batch-dispatcher" only in the defensive vanished-Batch corner), and it carries `_terminal_hash_facts(job)` (MUST-1's helper, built to never raise) so a degraded sanitize event keeps the artifact hashes the offline verifier cross-checks packets against. `_finalize_child_release` gets a single retry, log-only on persistent failure: every input is already durable and boot-time reconciliation (`_sweep_orphaned_jobs`, `_reconcile_stale_releases`) independently syncs any Release that disagrees with its terminal Job -- the audit event is the thing with no other writer. Audit-first ordering preserved: release sync can never cost the audit anything; `check_batch_completion` still always runs (finally). Two regression tests: an always-raising `_append_child_audit` must still leave a degraded event (`finalization_degraded=true`, truthful payload, actor = `batch.requested_by`) without breaking chain verification -- fails without the fix (`assert []`: the event list was empty, the audit event vanished); a raise-once `_flaky` must yield the normal, non-degraded event -- the retry exists so degraded is reached only by persistent failure.
- **SHOULD-3, certificate renders legal bases (`9dc7d7b`, `service/app/main.py` + `tests/test_certificate.py`)**: the certificate builder accepted `legal_justification` data in the manifest and release packet, but `_render_job_certificate_html` never received it -- the certificate, the one artifact a receiving party actually reads, never stated the legal basis for content that survived sanitization. It gains a `legal_justifications` parameter rendering a "Legal basis for retained content" section between the Manifest actions and Findings, one entry per kept finding (subtype, action, basis, note), every dynamic value `html.escape()`d like the rest of the certificate. Sourced in `_build_certificate_html` through the same `_legal_justifications_from_manifest` helper the release-packet and result builders use -- one projection, so `certificate.html` embedded in a packet matches `release_packet.json`'s `legal_justifications`. Inspect jobs produce no section (empty list), same as `policy_html`. The honesty rule carries over from the packet: when every entry carries only the engine's `unspecified` fallback, the section says so explicitly ("No operator-supplied legal basis was recorded ... not a legal determination") instead of presenting unspecified as a basis. Five regression tests: supplied bases render with no caveat; all-unspecified renders the caveat; nothing-kept omits the section; a `<script>`-injection note stays escaped; the bundle-embedded `certificate.html` carries the section. Known data-entry gap, flagged back per the brief and out of scope here: neither the Airlock CLI's `Client.release()` nor the web UI ever sent `legal_justifications` (the UI posted `finding_decisions` only) -- closed next, as PR 60.
- **SHOULD-4, dead client wrapper deleted (`53b886c`, `tools/counselclear_airlock.py` + `docs/COUNSELCLEAR_ASSET_MAP.md`)**: `Client.sanitize()` -- the caller-side wrapper for the legacy unwrapped `POST .../sanitize-jobs` route -- had zero callers: `run_airlock()` itself calls `Client.release()` (since PR 43), the web UI never touches the CLI, and no test imported it. Deleted the method, its docstring line in the module header, and the stale "retained for compatibility" line in the asset map. The server-side `/sanitize-jobs` route itself is untouched -- deleting a route is an API-removal decision, not dead-code cleanup; only the never-called client wrapper is gone.

Each fix's fail-without/pass-with proof was re-verified personally after review, by surgical revert-retest: cancel guard removed -> the race test fails with `'failed' == 'running'`; degraded-audit path removed -> the audit test fails with `assert []`. Both proofs fail only on the exact behavior they claim to pin. Full backend suite green on the combined result (1267 collected: 1266 passed, 1 pre-existing skip, 0 failures), `ruff` clean across service/tests/tools. Frontend untouched by three of the four; no frontend gates needed beyond the suite itself.

### PR 60 -- Legal-basis data-entry: Airlock CLI flags and web UI picker close the gap PR 55's chain had -- implemented (2026-08-29)

PR 55/56/57 built the legal-basis chain end to end -- schema, packet, certificate -- and PR 59's SHOULD-3 rendered it in the certificate. But no client ever *sent* a legal basis, so every real-world release still showed the certificate's "no basis recorded" honesty caveat: the evidence chain existed and stood empty. This pass closes both client halves, in three commits.

- **Airlock CLI (`b25a691`, `tools/counselclear_airlock.py` + `tests/test_airlock_cli.py`)**: `--legal-basis SUBTYPE=BASIS`, repeatable, plus one shared `--legal-basis-note`, parsed by `parse_legal_basis_flags()` into the `{subtype: {basis, note}}` shape `POST .../releases` takes. Both vocabularies are validated at argument parsing (`LEGAL_BASIS_VALUES` / `LEGAL_BASIS_SUBTYPES`, deliberate literal copies per the CLI's no-control-plane-import rule) so a typo fails before any network call rather than after the upload round trip. Omitting both flags is fully supported and unchanged -- surviving findings are recorded as basis `unspecified`, never gated. An orphan `--legal-basis-note` (no basis to attach to) is refused rather than silently dropped. `Client.release()` gains `legal_justifications=`, forwarded by `run_airlock()` and `run_airlock_batch()` (one basis set applies to every file in a batch, the same no-per-file-overrides rule as profile/recipient); the payload key is omitted entirely when none is supplied. Seven new tests: parse normalization, empty contract, rejection paths (main() rejects a bad basis and a bad subtype before any network call, both modes), FakeClient/FakeBatchClient capture the forwarded dict verbatim and absent when not supplied, batch passes it to every release call. The end-to-end test drives the real HTTP path: `hidden.xlsx`'s `hidden_structure` finding survives `counterparty_deal_room` as a flag record; `--legal-basis hidden_structure=privilege --legal-basis-note ...` lands in the extracted `certificate.html` ("Legal basis for retained content", privilege, the note, no unspecified caveat) and `release_packet.json`'s `legal_justifications`, and the packet still passes the offline verifier.
- **Web UI picker (`ab0913b`, `web/lib/legalBasis.ts` (new) + `web/lib/types.ts` + `web/app/matters/view/page.tsx` + `web/lib/legalBasis.test.ts`)**: ReleasePanel posted `finding_decisions` only. The basis picker bolts onto the panel's existing per-subtype Approve/Keep review with the panel's own primitives (native select, label maps, disclosure copy). `legalBasis.ts` is extracted like `productionReview.ts` so the logic is unit-testable without rendering React: `LEGAL_BASIS_LABEL`/`legalBasisLabel()` (human-readable labels over the controlled vocabulary, `RECIPIENT_TYPE_LABEL`'s exact pattern -- an unknown slug renders as itself, never disappears); `buildLegalJustifications(decisions, bases)` builds the POST's `legal_justifications`, where only KEPT subtypes get an entry -- a basis for a row set to "Approve (strip)" is dropped, because the backend would silently ignore it and the payload must not claim a basis for content that was stripped; `unspecified` picks are not sent; the field is omitted entirely when nothing applies -- a release with no supplied basis stays fully valid, never gated; notes truncate to the backend's own 1000-char cap. `FALLBACK_LEGAL_BASIS_DISCLOSURE` covers the no-inspect-results fallback path, where no per-finding UI exists: kept findings are recorded with no basis and the certificate will show them as unspecified, and the pre-submit state says so, in the same register as the caveat itself. `types.ts` gains `LEGAL_BASIS_VALUES` (the backend's 10-value enum as a deliberate second literal, `KNOWN_PERMS`' pattern -- vocabularies are shared by duplication, never by import). In ReleasePanel, each kept decision row gains a basis select + optional note input, rendered only while the row's decision is "keep" (a basis is semantically about retained content); `submit()` spreads `legal_justifications` into the POST beside `finding_decisions`. The fallback gate renders the disclosure. No new requirement anywhere: unspecified stays a valid state. Nine vitest pin the payload rules: kept+basis+note; empty contract (nothing picked -> undefined); dropped approved-row bases; undecided-means-keep default parity with the decision payload; empty-note basis still sent; 1000-char truncation; the label map covers exactly the controlled vocabulary (an extra entry would advertise a basis the backend rejects); raw-slug fallback.
- **Backend e2e pin (`d28bda6`, `tests/test_certificate.py`)**: `test_certificate_legal_basis_release_route_payload_matches_the_web_ui` posts to `POST .../releases` the exact body ReleasePanel.submit() now builds for the picker state "one subtype kept with a chosen basis + empty note, the other approved": `finding_decisions` plus `legal_justifications` with a real basis and an EMPTY note. Proves on the real API that an empty note with a real basis is recorded as basis + empty note on the certificate -- not the engine's unspecified fallback, no all-unspecified caveat, no note-dash rendered -- the empty-note case the vitest helper-level tests can only assert structurally.

Full backend suite green (1274 passed, 0 failures), `ruff` clean; frontend gates green (`tsc`, `eslint`, `vitest` 55/55, `next build`). Design decisions worth carrying forward: a basis rides ONLY surviving findings server-side (keep/flag/inspect_only records -- a basis on a stripped subtype is silently ignored, which is why the UI drops approved-row bases client-side and the CLI takes the subtype explicitly), and the certificate renders raw subtype slugs in `<code>`, not prettified labels.

### PR 61 -- Release-flow UX pass: mechanical fixes from the operator-eye audit -- implemented (2026-08-29)

An operator-eye audit of the matter -> job -> release -> packet flow (walked personally through `web/app/matters/view/page.tsx` and `web/app/matters/job/page.tsx` plus a sweep of every other route), producing numbered findings each rated mechanical vs. needs-a-decision. The honest verdict: post-PR-48 the flow is in good shape -- the findings were real but small. The five mechanical fixes landed as five separate commits; the six needs-a-decision items were reported, not implemented, and four of them were decided and landed the next day (PR 62).

- **Stale anchor copy (`d046218`)**: PR 57 made every release packet Ed25519-signed, but all three UI surfaces carrying anchoring language still described the packet purely by absence ("not externally anchored yet") and never mentioned the signature -- and the offline verify command shown to operators omitted the `--public-key` step the signature exists for. The job page packet box and BundleContents bullet now name the Ed25519 signature first, then state external anchoring as the specific remaining limitation (an operator signature is not an external timestamp authority -- the distinction the verifier itself documents); the verify command gains the `--public-key` invocation with the custody-public-key link and an honest note that the key must reach the recipient out of band (the route's own docstring: the trust root is the operator's transmittal, not the HTTP response); the view page's ReleasePanel Technical details matches. Wording stays inside the proposal's §7 forbidden-claims list: no "unforgeable", "independently timestamped", "court-proof".
- **Legacy sanitize wording (`3df92da`)**: after PR 40 renamed the action everywhere in the UI, the legacy no-`release_id` branch of the document next-step line still told operators to "Choose a policy and sanitize when ready" -- a verb and a noun the release flow no longer offers as a primary choice -- and the fresh-inspect state still read "Inspected -- not yet sanitized". Now "Inspected -- not yet released", with "Prepare a release packet when ready" (naming the real button). The legacy path keeps its own distinct labels (never claims a Release row exists -- that's a real data boundary), and the in-flight/done legacy arms keep "Sanitize"/"Sanitized with" -- those describe a job that genuinely ran through the legacy `/sanitize-jobs` route, not an instruction. `documentNextStep`/`STATUS_TONE_CLASS`/`STATUS_TONE_LABEL` were extracted from `view/page.tsx` into `web/lib/documentNextStep.ts` (~120 lines of branch logic with operator-facing wording in every arm, previously untestable inside the page component) with a 7-vitest suite pinning release-vs-legacy vocabulary separation, the not-yet-released wording, the inspected-again-since-release case, refusal wording distinctness, and that the legacy path never renders "Released under"/"release result".
- **Release button tooltip (`89b1241`)**: the "Prepare Release Packet" button is disabled without the sanitize grant (what `POST .../releases` actually checks), but its tooltip read "You don't have sanitize permission on this matter" -- naming a control the operator can't see anywhere in the UI, next to a button that never says "sanitize"; a denied user got no indication what was blocked or which grant to request. New `releaseGate(perms)` helper beside `permissionGate`: same sanitize grant underneath, but the denied tooltip says "You don't have permission to prepare a release on this matter (requires the sanitize grant)" -- the action first, then the grant that governs it. `permissionGate`'s generic per-perm message is untouched (Inspect/Upload tooltips still name their perms, which match their button labels). Three vitest: governed by sanitize, names the action plus the grant when denied, no tooltip when allowed.
- **422 crash (`bcf9a7b`)**: the api client read error bodies as `{ detail?: string }`, but FastAPI sends `detail` as an ARRAY of `{loc, msg}` objects for request-validation 422s. The array was assigned into `Error.message` and flowed to every error-rendering consumer, where React throws "Objects are not valid as a React child" -- a malformed release submission blanked the panel instead of showing the validation error. `request()` now normalizes at the one boundary every error passes through: plain-string detail kept as-is (HTTPException, the common case); array detail flattened to "body.<field>: <msg>; ..." (the field path plus the human-readable constraint); anything else keeps `statusText`. `Error.message` is always a string. Four vitest (mocked fetch): string detail passthrough, 422 array flattening, non-detail JSON body, non-JSON body. Fail-without proven by surgical revert: the 422-array test fails with the old handling, passes with the new.
- **Dashboard labels (`eb3247f`)**: three one-concept-two-names cases, each aligned with the wording every other surface already uses. JOBS BY STATUS rendered raw enum words ("done · 3") directly under a "Completed jobs" totals card meaning the same thing -- chips now read "Completed / Failed / Refused / Queued / Running" with the same counts. Recent activity labeled the `bundle.download` event "Downloaded bundle" while the job page and matter view call the same artifact "release packet" everywhere -> "Downloaded release packet". The unreviewed-findings attention row's link said "Open certificate" while the job page's identical link says "Open custody certificate" -> matched.

Full backend suite green (1277 passed, 1 skipped, 0 failures), `ruff` clean; frontend gates green (`tsc`, `eslint`, `vitest` 69/69, `next build`). Reported but deliberately not implemented in this pass (needs-a-decision items): no re-run path from a refused job's page; the dashboard's "Failed / refused jobs" card dead-ending at the matters list; audit-page raw action strings vs. the dashboard's humanized labels; timestamp format inconsistency across surfaces; recent-activity rows linking only to the matter; login landing on /matters while / redirects to /dashboard.

### PR 62 -- Decided UX items: re-run panel, cross-matter problem-jobs list, unified landing, activity deep-links -- implemented (2026-08-29/30)

Four of PR 61's six needs-a-decision items, decided by the operator and implemented the next working session; each its own commit. The two left open: audit-page raw `job.sanitize` action strings vs. the dashboard's humanized labels, and the timestamp format inconsistency across surfaces.

- **Login lands on the dashboard (`947de96`)**: `app/page.tsx` redirected `/` to `/dashboard`, but a successful login redirected to `/matters` -- two different answers to "where do I land" depending on entry path. Login now lands on `/dashboard`, matching the root redirect; Matters stays one click away in the header nav. Verified before changing: no test pins the login destination, no `?next`/`returnTo` handling exists in the login flow (the redirect is a hard-coded string), the logout -> `/login` and 401-bounce -> `/login` flows are unaffected, and no doc claims login lands on `/matters`.
- **Cross-matter failed/refused jobs list (`8e28c81`)**: the dashboard's "Failed / refused jobs" card linked to `/matters` with no way to see just those jobs across matters. New `GET /v1/jobs` -- read-scoped with the same disclosure argument as the dashboard's refused/failed attention queues (every row's detail is already visible through the read-gated per-job route), comma-separated status allowlist with 400 on unknown words, limit/offset capped server-side, demo matters excluded like the dashboard. Rows are deliberately AttentionItem-shaped (`type` = terminal status, plus `release_id`/`profile_id` for the re-run flow) so the frontend reuses the existing link machinery instead of a parallel rendering path. New `/matters/jobs` page: refused/failed tabs as server-side filters (status keyed into the pagination list key so switching tabs resets to page 1), rows link to the exact job via `attentionPrimaryHref`, and the dashboard card is repointed from `/matters` to `/matters/jobs`. Link helpers' parameters narrowed to the structural `AttentionLinkTarget` they actually need; `?status=` deep-links resolve through `resolveProblemJobFilter`, which forwards only values the page itself would send. Five backend tests -- the cross-matter proof seeds two matters with mixed statuses and asserts both appear, done jobs excluded, newest first, plus release/profile ids on wrapped rows, filter select + 400-reject, ACL scoping + demo exclusion, and read-only vs. stranger principals. Seven frontend vitest: every tab value decomposes into endpoint-legal statuses, the default equals the card's destination, and param resolution falls back on 400-bait strings. (Test-seeding lesson recorded in the commit message: the session factory's context-manager exit rolls back without an explicit `s.commit()` -- flush-only helpers leave the app's own session seeing an empty database.)
- **Re-run panel for refused/failed jobs (`68814fe`)**: the dashboard's refused-attention copy promises "re-run with a different policy or attestation," but the job page it links to dead-ended at certificate/result links. A collapsed `<details>` panel below the refusal callout -- the verdict stays the primary signal; a re-run is a new custody event, not a "try again". Always `POST .../documents/{id}/releases`, the only user-facing path, even for a legacy refused job: its `policy_id` maps to the release profile that resolves to the same policy (`pickPrefilledProfile`), with the substitution disclosed when no profile matches. Prefills from the actual Release row (profile, recipient type/name, purpose, intended-external) and the Job payload's own attestation bool. Mount-gated until job + release + profiles resolve so the prefill doesn't freeze to defaults (the same gated-fetch pattern the manifest render already uses); a profiles-fetch error surfaces as an inline message instead of silently dropping the panel. An exact-phrase hint appears when the refusal was specifically the missing signature-break attestation and the checkbox is still unchecked -- never for macros refusals the checkbox can't save. Permission-gated with the same `releaseGate` tooltip as the matter page. Two gaps caught in lead review and fixed before commit: recipient name and purpose were prefilled and POSTed but never rendered (invisible values on a custody record -- inputs added, matching the matter view's own labels), and the dead `profilesError` prop was removed once the fetch-error path rendered outside the panel. `rerunRelease.ts` holds the prefill/mapping/payload logic as pure functions (logic in lib, JSX in the page), pinned by 14 vitest including the visible-prefill contract.
- **Recent-activity deep-links (`0dad6fd`)**: recent-activity rows all linked to the bare matter view, even when the underlying audit event named the exact job or document it was about. Backend: `recent[]` now echoes `job_id`/`document_id` from each event's AuditEvent payload, only when that action type actually records them -- grounded in the append sites: `job.inspect`/`job.sanitize`/`release.created`/`certificate.issued` carry both; `release.terminal`/`bundle.download`/`attest.used` job only; `document.upload`/`attest.issued` document only; `matter.create`/`acl.grant`/`acl.revoke`/`batch.*` neither. Only real non-empty strings are echoed -- a malformed payload degrades to a matter link, never a `?job=null` href. Frontend: `recentActivityHref` picks the most specific destination -- the exact job page (the same URL shape `attentionPrimaryHref` builds, so the two feeds share one link convention), the matter view scrolled to the document (`?doc=`, same as `attentionMatterHref`), or a plain matter link when the action has nothing more specific to point at. Backend test pins each action's echoed ids plus the malformed-value drop; three frontend vitest pin the three link tiers and the convention match.

Full backend suite green on the combined result (1280 passed, 1 skipped, 0 failures), `ruff` clean; frontend gates green (`tsc`, `eslint`, `vitest` 93/93, `next build` -- 12 routes including the new `/matters/jobs`).

### PR 63 -- Bugfix: DOCX highlight-only concealment survived "strip" under external_sharing

A real user-reported release failure, investigated as a correctness bug rather than user error. Reported symptom: preparing a release packet for a DOCX carrying one directly-highlighted run (`<w:highlight>`, no `w:vanish`, no white-on-white text) under `counterparty_deal_room` completed `failed`, with `verification failed: reinspect_targeted_gone`; direct engine reproduction confirmed the failure and isolated it before any code changed.

- **Root cause, confirmed not assumed**: the detector (`container_meta.py`'s `_inspect_docx_legal`) correctly flagged `hidden_text` from the `<w:highlight>` occurrence, and `external_sharing`'s policy correctly planned `strip` for it -- but the remover (`_docx_strip_hidden_text`/`_walk_hidden_runs`/`_run_is_hidden`) only ever resolved `w:vanish`; it had no code path for highlight at all. "Strip" silently did nothing, `reinspect_targeted_gone` correctly noticed the promised-gone finding was still present on re-inspect, and refused the release -- the fail-closed gate working exactly as designed over a promise the remover could never keep. Not specific to this document: any DOCX with ordinary Word highlighting (a common attorney-review habit) hit the identical dead end under any policy resolving `hidden_text` to `strip`.
- **Fix, formatting-only by requirement**: new `_docx_strip_highlight` (`container_meta.py`), deliberately independent of `DocxHiddenModel`/`_walk_hidden_runs`/`extract_docx_hidden_text` -- a highlighted run is visible, not concealed, and must not enter that "hidden" vocabulary. Removes only the `<w:highlight>` element from a run's direct `w:rPr` (never `w:t`, never the run, never a style-resolved highlight -- direct-formatting only, matching the same scoping decision already accepted for white-on-white detection); a `w:val="none"` run is left untouched. Wired into `clean_docx`'s existing `strip_hidden_text` gate, vanish-first then highlight-second on the same bytes, with no signature changes anywhere in the call chain -- the existing `hidden-text:`-prefixed action-message classification already routes the new message to the `hidden_text` `ActionRecord` subtype.
- **Applied-vs-raw split extended to highlight, and fixed for white at the same time**: `_inspect_docx_legal`'s `highlight` (and pre-existing `white`) counters included every `<w:highlight>`/`FFFFFF` declaration anywhere in `word/*.xml`, styles.xml and numbering.xml included -- machinery, not concealed or marked-up text. New `_DOCX_APPLIED_HIGHLIGHT_RE`, scoped to `w:rPr` in a body-bearing part exactly like the existing `_DOCX_APPLIED_WHITE_RE`, feeds new `hidden_highlight_applied_runs`/`hidden_highlight_applied_parts` report fields. The finding *trigger* moved from `if vanish or white or highlight` to `if vanish or white_applied or highlight_applied` -- gated on applied counts for both signals now, not just reported that way for white as before. Deliberately not scoped to highlight alone: leaving white's trigger on the raw count would have shipped two different closure semantics for two structurally identical fields in the same finding string, and the raw-count trigger was a latent, real instance of the identical bug class (undemonstrated only because literal styles.xml noise is rarer for `w:highlight` than `FFFFFF`) -- confirmed with the user before broadening the fix rather than assumed.
- **Override reused unchanged**: `policies.py`'s existing `decisions.get("hidden_text") == "keep"` downgrade (`{"action": "flag", "reason": "operator_acknowledged"}`) is keyed on the `hidden_text` subtype, not on which underlying signal triggered it -- `_FINDING_SUBTYPE_ALIASES["hidden_text_formatting"] = "hidden_text"` already routes highlight-caused findings into the same bucket, so a highlight-only document reaches the identical override with zero code changes. `ReleasePanel` (`web/app/matters/view/page.tsx`) gained a "keep hidden-text formatting instead of stripping it" checkbox -- previously only `production`'s per-finding grid could decline a hidden_text strip; `external_sharing` (the only other named policy that strips it outright) had no control for it at all.
- **Copy**: `POLICIES`/`RELEASE_PROFILES` descriptions (`service/app/main.py`) updated to name the new hidden-text-formatting strip explicitly; `external_sharing`'s `POLICIES` description also corrected a pre-existing inaccuracy found in passing (it said hidden content was "flagged," which has been wrong since hidden_text was flipped to `strip`).
- **Tests**: 8 new tests in `tests/test_custody_truthfulness.py` (`test_highlight_only_concealment_is_stripped_not_deleted` is the core-requirement test -- highlighted words survive byte-for-byte, only the mark is gone; `test_verify_passes_for_highlight_only_document_after_strip` is the end-to-end closure test that reproduces and proves the fix, the one that would have caught the original bug), 1 in `tests/test_docx_legal.py`, plus an update to `test_style_level_white_is_not_reported_as_concealed_text` for the broadened trigger. Golden snapshot fixtures (`tests/fixtures/legal/golden/{spa.docx,macro.docm}.json`) regenerated via `UPDATE_GOLDENS=1`; diff confirmed purely additive (the two new report fields only).

Full backend suite green (1429 collected, zero regressions), `ruff check`/`ruff format --check` clean on touched files; frontend gates green (`tsc`, `eslint`, `vitest` 124/124, `next build`). End-to-end verified against the actual reported document, outside the test suite: `inspect_bytes` -> `plan_actions("external_sharing")` -> `apply_actions` -> `verify_derivative` went from `reinspect_targeted_gone` failing to the full gate passing, with the derivative's `word/document.xml` run-text identical to the original except five pre-existing, unrelated Layer-A NBSP normalizations.
