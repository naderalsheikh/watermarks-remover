...
- **Airlock CLI** (`tools/counselclear_airlock.py`) — Release-native since PR 43 (`Client.release()`, `--profile`, required `--recipient-type`). The CLI's legacy `Client.sanitize()` / `--policy` path was removed as dead code (it had no callers); the server-side legacy `/sanitize-jobs` route is unaffected (see "Legacy routes" below).
