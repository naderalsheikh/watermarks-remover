...
This document preserves the legacy upstream `watermarks-remover` utility and related research-harness documentation that previously lived in the top-level README. It is retained for engineering reference and research work, not as the primary CounselClear product entry point.
...
See [docs/windows-autostart.md](windows-autostart.md) for auto-starting the service at Windows login without Docker.
...
[`docs/synthid-text-benchmark.md`](synthid-text-benchmark.md).
...
- **PDF embedded-image metadata detection and byte-preserving removal** (`inspect_pdf`/`clean_pdf` in `service/scripts/container_meta.py`): a PDF page that's really a scanned/photographed JPEG previously kept its image's own EXIF/GPS/C2PA even after a "successful" document-level clean, which only ever touched `/Info` and XMP, never image XObjects. `inspect_pdf` now surfaces it as a finding; `clean_pdf` strips it by splicing the identified metadata segments directly out of the extracted JPEG stream and re-running qpdf's structural rewrite — the DCT-coded scan data is never decoded or re-encoded, so there's no recompression risk. A Ghostscript re-encode pass was tried first and rejected: with every "don't degrade the image" flag it offers set, it reliably dropped the embedded image entirely rather than stripping just its metadata; see `docs/pdf-deep-image-metadata.md` for the investigation. One known gap: an image whose `/Length` is an indirect reference is skipped rather than guessed at (rarely reachable once qpdf has normalized it, which clean_pdf's own upstream rewrite already does when qpdf is available)
- **CounselClear production-hardening review fixes**: eight findings from review of the hardening pass fixed — `/v1/auth/oidc/callback` gets the same per-peer brute-force throttle as local login; `/health` split into a dependency-free liveness check and a DB-checking `/health/ready` (orchestrator liveness probes no longer restart the process over a transient database outage); the OIDC `sub` allowlist match is case-sensitive per spec (only `email` is case-folded); OIDC CSRF-state signing now reuses the same HMAC helper as session cookies instead of a second hand-rolled implementation; `revoke_all_sessions` rotates the cookie secret with an atomic `os.replace()` instead of unlink-then-recreate, closing a window where a concurrently-issued session could be silently invalidated before its cookie reached the client; the startup orphan-job sweep is one bulk `UPDATE` instead of a per-row ORM loop; the Postgres audit-chain append retry now rolls back only its own attempt (via `SAVEPOINT`/`begin_nested()`) instead of the caller's whole transaction, so a seq collision can no longer silently drop an already-staged Matter/Document/ACL row; and an ASGI deployment with no visible peer address (e.g. bound to a Unix socket) now logs a one-time warning that the login throttle and access log are sharing one bucket across every caller instead of failing silently
- **CounselClear deployment kit** (`deploy/`): worked example artifacts for the production topologies in `docs/COUNSELCLEAR_PRODUCTION.md` — an nginx TLS proxy config (256m upload cap matching the engine limit, login rate-limit zone, docs endpoints 404'd at the edge, `X-Forwarded-Proto` so session cookies keep their `secure` flag), a hardened systemd unit for running `cc-api` as a native host process (the topology that gets real per-job docker/gVisor isolation without mounting the Docker socket anywhere), and a gVisor `daemon.json` registration snippet. All `.example` files — copy, fill in names/paths, review against §7's checklist
- **CounselClear PDF visual compare (PR 14) and `qpdf --check` derivative validation landed**: every PDF derivative now gets a `qpdf --check` structural pass as part of the verification gate (exit 0/3 passes, 2 fails; degrades to magic-byte-only when qpdf isn't installed), and an optional render-and-compare rasterizes original vs derivative page-by-page with poppler at 150 dpi — mean absolute per-channel pixel delta after a 1px box blur, annot/acroform bboxes masked out of the denominator, first-10/last-5/sampled-middle page cap of 30. Strictly warn-only: a high delta lands in the manifest under `verification.visual_compare` for operator review and never fails the job. Off by default (`COUNSELCLEAR_VISUAL_COMPARE=1` to enable); no Office/LibreOffice compare (missing firm fonts would dominate false positives)
- **CounselClear compose `legal` profile now defaults `COUNSELCLEAR_WORKER_MODE` to `subprocess`, not `docker`**: the old default was silently non-functional — the containerized `cc-api` has no docker CLI or socket, so every docker-mode job would have failed immediately. Docker-outside-of-docker (mounting the host socket into `cc-api`) was considered to fix this and explicitly rejected: it would give any `cc-api` compromise host-root-equivalent access, defeating the per-job isolation and custody guarantees it's meant to protect. Real per-job docker/gVisor isolation now requires running `cc-api` as a native host process instead of the containerized service — see `docs/COUNSELCLEAR_PRODUCTION.md` §3
- **CounselClear production deployment guide** (`docs/COUNSELCLEAR_PRODUCTION.md`): topology, digest pinning, per-job worker sandboxing with an optional gVisor runtime (`COUNSELCLEAR_WORKER_RUNTIME=runsc`, now a supported runner option), managed Postgres, S3 Object Lock + CMK + region residency guidance for the custody store, OIDC setup, and an operations checklist
- **CounselClear optional OIDC SSO (Phase 3)**: set `COUNSELCLEAR_OIDC_ISSUER`/`CLIENT_ID`/`CLIENT_SECRET` to replace the shared local password with single sign-on — signed-state CSRF protection, JWKS-verified RS256 ID tokens, a fail-closed email/subject allowlist, and per-identity principals (`oidc:<hash>`) isolated by the existing matter ACLs; local login is disabled while SSO is on
- **CounselClear optional Postgres backend (Phase 3)**: set `COUNSELCLEAR_DATABASE_URL` to run the control plane against Postgres instead of embedded SQLite — shared state for multi-replica deployments, JSONB columns, an audit-chain append that survives cross-process seq races via bounded retry, and a healthchecked `cc-postgres` service under compose's new `pg` profile. SQLite remains the default; Alembic renders real JSONB DDL for fresh Postgres installs
- **CounselClear custody storage backends — S3 Object Lock, CMK, residency pin (Phase 3, PR 21)**: `app/storage.py` adds the original-custody storage boundary — `COUNSELCLEAR_STORAGE=s3` switches to S3-compatible object storage with per-object Object Lock `COMPLIANCE` retention (write-once via `If-None-Match: *`); `COUNSELCLEAR_CMK_ARN` (AWS KMS) or `COUNSELCLEAR_VOLUME_KEY_FILE` (0600 file) turns on transparent AES-256-GCM envelope encryption around either backend; `COUNSELCLEAR_RESIDENCY_REGION` pins the bucket's region and refuses to boot on a mismatch. Everything defaults to the unchanged local O_EXCL+0444 write-once path. This closes out Phase 3 for v1's single-tenant scope — see `docs/COUNSELCLEAR_DESIGN.md`
- **CounselClear web UI (Phase 4, PR 19)**: `web/` is a Next.js reviewer UI — login, matters, per-document inspect/sanitize with a policy picker, category-grouped findings, and bundle download. Same-origin static export by design (the `cc_session` cookie is `SameSite=Strict`): no Next.js server in production, nginx serves `web/out` at `/` and proxies `/v1/*` to `cc-api` (`deploy/nginx-counselclear.conf.example` updated to match). Landing it added the `GET` list endpoints the API was missing for matters/documents/jobs, a public `GET /v1/auth/config` and `GET /v1/policies`, and fixed the OIDC callback to redirect into the app instead of returning raw JSON. See `web/README.md`
- **CounselClear single-tenant production hardening**: orphaned queued/running jobs are failed at startup instead of sitting "running" forever after a crash; SQLite gains WAL, `busy_timeout`, and enforced foreign keys; the login route gained a per-peer brute-force throttle (429 + Retry-After) plus logout and server-side session revocation (`/v1/auth/revoke-sessions` rotates the cookie secret); `/health` is a dependency-free liveness check and the new `/health/ready` checks the database (the compose healthcheck now targets `/health/ready`); `/docs`+`/openapi.json` are fail-closed behind `COUNSELCLEAR_ENABLE_DOCS=1`; requests log one JSON line each with an `X-Request-ID` correlation header; a `cc-freshclam` sidecar keeps ClamAV definitions current; incomplete job bundles return 409 instead of 500; CI's pip-audit now covers the shipped `service/requirements-app.txt`; and alembic no longer clobbers the host process's logging config on boot. See `docs/COUNSELCLEAR_DESIGN.md` ("Production-readiness pass")
