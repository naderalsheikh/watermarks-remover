...
def test_airlock_cli_end_to_end_against_a_real_server(tmp_path, live_server):
    ...
    # PR 57 (MUST-2): packets are signed, so the anchor is the operator's
    # Ed25519 signature rather than "none". Since Lane E1 Phase 1 the
    # packet also publishes its public key, so a key-less run can check
    # the signature and reports `self_key` -- mathematically sound,
    # provenance unconfirmed -- instead of the old `no_key` downgrade.
