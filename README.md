...
certificate proving exactly what was removed and when. Every action is recorded
in a tamper-evident, hash-chained custody log — malpractice-grade proof that
the document you released is the document you meant to release. It is API-first
(built for future Clio/iManage integration) with a web UI a paralegal can use
without reading a manual.

> **Production / live deploy:** the pilot deploys as a single web service —
> see [`DEPLOY.md`](DEPLOY.md) and [`render.yaml`](render.yaml). Expected live
> URL: `https://clearcounsel.onrender.com`.
