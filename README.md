...
- **PDF embedded-image metadata detection** (`inspect_pdf`/`clean_pdf` in `service/scripts/container_meta.py`): a PDF page that's really a scanned/photographed JPEG now has its image's own EXIF/GPS/C2PA detected and reported — `inspect_pdf` surfaces it as a finding, `clean_pdf`'s manifest carries a `deep_images` field — where before it silently survived a "successful" document-level clean (which only ever touched `/Info` and XMP, never image XObjects). Detection only, deliberately: a Ghostscript removal pass was built and not shipped after it reliably dropped embedded images outright in testing rather than merely stripping their metadata; see `docs/pdf-deep-image-metadata.md` for the investigation and the follow-up plan (byte-preserving stream surgery instead of a re-render is the leading candidate)
