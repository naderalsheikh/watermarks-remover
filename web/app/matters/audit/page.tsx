...
function AuditView({ matterId }: { matterId: string }) {
  ...
  const linked = auditQ.data ? chainContinuity(auditQ.data.events) : new Map<string, boolean>();
  const anyLinkBroken = [...linked.values()].some((ok) => !ok);
  const visible = (auditQ.data?.events ?? []).filter(
      ...
      {auditQ.data && (
        <>
          ...
          {/* Independent of chain_ok above (which is the server's own
              recomputation): this is the same continuity check run again
              in the browser against the rows the server actually sent,
              so a compromised or bugged server response can't just claim
              chain_ok: true over a list that doesn't actually link up. */}
          {anyLinkBroken && (
            ...
            <>
              ...
              {categoryFilter !== "all" && (
                <p className="mb-2 text-xs text-muted">
                  Showing {visible.length} of {auditQ.data.events.length} events — the full
